function initializeProjectCreateWizard(form) {
    if (form._projectWizard) {
        return;
    }
    ...
    function requestStep(targetStep) {
        if (
            ...
            validateProjectName(form);
            validateProjectDescription(form);
            return;
        }
...
function validateProjectName(form) {
    ...
    htmx.ajax(
        ...
        {
            source: form,
            target: "#project-create-name-field",
            swap: "outerHTML",
            values: {
...
function validateProjectDescription(form) {
    ...
    htmx.ajax(
        ...
        {
            source: form,
            target: "#project-create-description-field",
            swap: "outerHTML",
            values: {
...
function validatePreferredImage(form) {
    ...
    htmx.ajax(
        ...
        url,
        {
            source: form,
            target:
                "#project-create-preferred-image-field",
            swap: "outerHTML",
            values: {
...
function initializeProjectCreateWizards(root) {
    if (
        root.matches?.(
            "[data-project-create-wizard]"
        )
    ) {
        initializeProjectCreateWizard(root);
    }

    root.querySelectorAll?.(
        "[data-project-create-wizard]"
    ).forEach(
        initializeProjectCreateWizard
    );
}
...
document.addEventListener(
    "DOMContentLoaded",
    () => {
        initializeProjectCreateWizards(
            document
        );
    },
);
...
document.body.addEventListener(
    "project-create-description-valid",
    () => {
        const form = document.querySelector(
            "[data-project-create-wizard]"
        );
