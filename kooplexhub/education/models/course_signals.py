...
def create_course(sender, instance, **kwargs):
    ...
    FilesystemTask.objects.create(
        folder = course_public(instance),
        groups_ro = code([group]),
        create_folder = True,
        task = FilesystemTask.TSK_GRANT
    )
    FilesystemTask.objects.create(
        folder = course_assignment_prepare_root(instance),
        create_folder = True,
        task = FilesystemTask.TSK_CREATE
    )
    FilesystemTask.objects.create(
        folder = course_assignment_snapshot(instance),
        create_folder = True,
        task = FilesystemTask.TSK_CREATE
    )
    FilesystemTask.objects.create(
        folder = course_workdir_root(instance),
        create_folder = True,
        task = FilesystemTask.TSK_CREATE
    )
    FilesystemTask.objects.create(
        folder = course_assignment_root(instance),
        create_folder = True,
        task = FilesystemTask.TSK_CREATE
    )
    FilesystemTask.objects.create(
        folder = assignment_correct_root(instance),
        create_folder = True,
        task = FilesystemTask.TSK_CREATE
    )
...
def add_usercourse(sender, instance, **kwargs):
    ...
    FilesystemTask.objects.create(
        folder = course_workdir(instance),
    ...
    if instance.is_teacher:
        FilesystemTask.objects.create(
            folder = course_public(course),
            users_rw = code([user]),
            task = FilesystemTask.TSK_GRANT
        )
        FilesystemTask.objects.create(
            folder = course_assignment_prepare_root(course),
            users_rw = code([user]),
            task = FilesystemTask.TSK_GRANT
        )
        FilesystemTask.objects.create(
            folder = assignment_correct_root(course),
            users_rw = code([user]),
            task = FilesystemTask.TSK_GRANT
        )
        for studentbinding in instance.course.studentbindings:
            FilesystemTask.objects.create(
                folder = assignment_workdir_root(studentbinding),
                users_ro = code([user]),
                task = FilesystemTask.TSK_GRANT
            )
            FilesystemTask.objects.create(
        ...
        dir_assignment_workdir = assignment_workdir_root(instance)
        FilesystemTask.objects.create(
...
def delete_usercourse(sender, instance, **kwargs):
    ...
    FilesystemTask.objects.create(
        folder = assignment_workdir_root(instance),
        remove_folder = True,
        task = FilesystemTask.TSK_REMOVE
    )
    FilesystemTask.objects.create(
        folder = course_workdir(instance),
        tarbal = course_workdir_garbage(instance),
        remove_folder = True,
